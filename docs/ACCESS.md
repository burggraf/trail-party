# P02 access contract — deny by default and current T2 boundary

This is the P02 schema/auth/security contract plus the currently implemented T1/T2 proof boundary; it is not a P02 phase-acceptance claim. [API](API.md) is the complete exposed surface and projection-column allowlist; [ARCHITECTURE](ARCHITECTURE.md#p02-design-contract--data-model-not-implementation) defines each table, FK, retention and version. T1 application tables/projections, native local-mail auth, validated browser session handling, the authenticated profile-create command and return-to helper now exist. P02.T3 authorization, avatar/device commands, the complete browser actor matrix and all gameplay remain pending. Games, teams, answers and reveals used by P02 tests are **synthetic fixtures**, not working gameplay. P05–P10 own their positive command/UI flows.
