...
## P01.T3 bootstrap gate

`pnpm test:bootstrap` is finite and requires real installed v0.33.14/SQLite 3.53.2 and Vite. Nineteen checks include actual health + nonce/schema readiness and `/display`, mode-0600 logs denied over frontend HTTP, stopped-depot synthetic SQLite data surviving restart, unmarked/wrong-owner/root-marker-tree symlink and external-target refusal, numeric/loopback validation, foreign listener/file noninterference, exact backend identity refusal, same-depot concurrent ownership, injected health-only/wrong-schema/child/spawn/frontend failures, SIGINT/TERM/HUP, stubborn version-probe timeout/cancellation with actual probe reaping, and owned-group escalation. Fault endpoints must prove both health and actual per-start schema requests reached them. No successful lifecycle is mocked. Structural decisions/release-pin/CI checks are documentation proof only.

Raw logs stay private/ignored; terminal errors expose only sanitized status and paths. Test workspaces alone are removed, after cleanup. Acceptance polling uses fixed state-based bounds (10s per launcher readiness phase, 20s test readiness/refusal, 8s shutdown including 5s owned-group escalation), zero retries/skips; no sleep-for-readiness. Normal development data persists. SIGKILL/machine failure can leave a stale lock, intentionally not stolen. Native shared-window/cleanup and remote Linux execution remain separate parent gates before C4.

The independent cleanup repair adds five negative-only `cleanup ownership` checks. They use exact launcher/backend PID-group IPC, an HTTP listening barrier, a backend SIGTERM release barrier and existing owned log-FD/depot seams; no positive stack is replaced. The current defective launcher was red (5 tests: 2 passed / 3 failed), then the repaired launcher was green (5/5, zero skips). The EPERM cases prove one transient signal-zero denial is followed by native disappearance, persistent probe denial retains the lock, and real signal-send denial remains nonzero/retained. Private diagnostics retain bounded original frozen Error/primitive reasons for frontend/http/backend/log.close/lock.remove without terminal leakage. The historical real SIGINT failure remains unresolved and is not accepted by these checks.
