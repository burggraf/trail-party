...
No direct client collection for private answer/key/grade/score/assignment material in P02, and no expand allowlist entries at all. Configured aliases use explicit excluded_columns plus allowlisted output schema; hidden `_` columns alone are insufficient. Tests must prove exclusion in read/list/filter/count/schema and actual filtered SSE insert/update/delete bytes, including denied scopes and membership/device revocation. If the pinned engine cannot safely enforce a projection or ongoing SSE authorization, that endpoint stays disabled and its gate stays failing until a minimal scoped server solution is proven; never publish a whole private table as a workaround.
