...
The private `server_secrets` table stores only named 32-byte server key material and is not mapped to a Record API. The profile handler creates its `trail-party/audit/request-hmac/v1` key with WASI-backed secure randomness on first use, initializes it with an insert-if-absent transaction, and reloads it after every process restart. It is never client-supplied, returned, logged or included in projections; losing the private database is a key-loss/recovery event, not permission to fall back to an unkeyed digest.
