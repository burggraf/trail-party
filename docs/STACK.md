...
`pnpm test:backend -- capabilities` builds a synthetic-only WASM component, starts real v0.33.14 on an ephemeral loopback port, migrates a marked `.local/test-runs/capabilities-*` depot, and disposes it. Config is textproto, migrations live in `migrations/main/`, and components load from `wasm/*.wasm`. A per-run public readiness API exposes only a constant row, proving depot identity before any auth mutation. The separate `auth` selector and `pnpm dev` path also build/install the narrow T2 profile component into their owned depots; `pnpm dev` additionally starts a fresh owned Mailpit v1.31.0 with a printed inbox URL, so local registration/verification does not require TrailBase admin credentials or the external Trailhead Mailpit. No gameplay WASM or production deployment is implied.
