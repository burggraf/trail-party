...
if (args.length !== 1 || !['capabilities', 'schema', 'auth', 'authorization'].includes(args[0])) {
  console.error('Usage: pnpm test:backend -- capabilities|schema|auth|authorization (owned local depot only)');
  process.exit(1);
}
...
const commands = args[0] === 'capabilities'
  ...
  : args[0] === 'schema'
    ...
    : args[0] === 'auth'
      ...
      : [
          ['pnpm', ['build:backend:probe']],
          [process.execPath, ['--test', 'tests/backend/authorization.test.ts']],
        ];
